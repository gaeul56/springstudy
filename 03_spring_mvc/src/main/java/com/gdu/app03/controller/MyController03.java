...
public class MyController03 {
	...
	public String blogDetail(HttpServletRequest request, Model model) {
		...
		model.addAttribute("blogNo", blogNo); //model을 사용하는 것을 권고
		return "blog/detail"; // /WEB-INF/views/blog/detail.jsp로 포워딩한다.
	}
	 ...
	 * 	1) 요청 파라미터를 필드로 가지고 있는 객체를 커맨드 객체라고한다
	...
	// @RequestMapping("/blog/detail.do")
	public String blogDetail3(BlogDto dto) { //Model에 저장된 이름은 dto가 아니라 blogDto이다.
		return "blog/detail";
	}
	
	/*
	 *@ModelAttribue 를 이용해서 Model에 저장되는 커맨드 객체의 이름을 지정할 수 있다. 
	 */
	
	@RequestMapping("/blog/detail.do")
	public String blogDetail4(@ModelAttribute("dto") BlogDto blogDto) { //Model에 저장도는 이름은 dto이다.
		return "blog/detail";
