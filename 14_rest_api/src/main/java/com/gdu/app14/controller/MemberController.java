 ...
 * 1. 요청주소를 작성하는 새로운 방식이다.
 * 2. 변수를 파라미터로 추가하는 Query String 방식이 아니다.
 * 3. 변수를 주소에 포함시키는 Path Variable 방식을 사용한다.
 * 4. "요청 주소 + 요청 방식"을 합쳐서 요청을 구분한다.
 * 5. 예시	
 * 				URL 		Method
 * 1) 목록	 /members  		 GET		/members/page/1 >> 1은 변수를 넣어줌
 * 2) 상세	 /members/1		 GET
 * 3) 삽입   /members 		 POST
 * 4) 수정   /members		 PUT
...
public class MemberController {
	...
	//회원 등록 요청 
	// JSON 데이터를 POST방식으로 본문에 포함시켜서 보낼 때 @RequestBody 이용해서 전달할 수 있음
	// JSON 데이터가 POST방식으로 본문(MemberDto)에 전달되는 것을 의미한다.
	...
	//회원 목록 요청
	//경로에 포함 되어있는 변수 {page}는 @PathVariable을 이용해서 가져올 수 있다.
	@RequestMapping(value="/members/page/{page}", method=RequestMethod.GET, produces="application/json")
	public List<MemberDto> getMembers(@PathVariable(value="p", required=false) Optional<String> opt){
